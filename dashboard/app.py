...
<style>
  ...
  [data-testid="stMetric"] {
      ...
      box-shadow: 0 4px 24px rgba(0,0,0,0.27), inset 0 1px 0 rgba(201,168,76,0.13);
  }
  ...
  [data-testid="stSidebar"] p { color: #8a9ab5 !important; }
  ...
  [data-testid="stButton"] > button {
      background: linear-gradient(135deg, #c9a84c, #a07c2a) !important;
  ...
  [data-testid="stButton"] > button:hover {
      ...
      box-shadow: 0 6px 20px rgba(201,168,76,0.27) !important;
  }
...
# Gold colorscale using valid rgba() strings (fixes the crash)
GOLD_SCALE = [
    [0.0,  "rgba(30,45,69,1)"],
    [0.5,  "rgba(140,100,30,1)"],
    [1.0,  "rgba(201,168,76,1)"]
]
...
# ── Helper: replace deprecated use_container_width ───────────────────────────
def pchart(fig, height=None):
    """Plot chart using the new width='stretch' API, with optional height override."""
    if height:
...
with st.sidebar:
    ...
    year_filter = st.selectbox("Year", ["All", "2020", "2021", "2022", "2023", "2024", "2025", "2026"])
...
with tab1:
    ...
    fig.add_trace(go.Scatter(
        x=mdf["month"], y=mdf["revpar"],
        fill="tozeroy", name="RevPAR",
    ...
    with c1:
        ...
        fig2.add_trace(go.Scatter(
            x=mdf["month"], y=mdf["occupancy_rate"],
    ...
    # ── Revenue bar — FIXED: use colorscale with rgb() format ──
...
with tab2:
    ...
    room_kpis = ddf.groupby("room_type").agg(
    ...
    c1, c2 = st.columns(2)
    with c1:
        fig = px.bar(room_kpis, x="room_type", y="avg_revpar",
            color="room_type", color_discrete_sequence=COLORS,
            title="RevPAR by Room Type ($)")
        fig.update_layout(showlegend=False, height=300, legend=LEGEND_STYLE, **CHART_LAYOUT)
        pchart(fig)
    with c2:
        fig2 = px.scatter(room_kpis, x="avg_occ", y="avg_adr",
            size="total_revenue", color="room_type",
            color_discrete_sequence=COLORS,
            title="Occupancy vs ADR — Bubble = Revenue",
            labels={"avg_occ":"Occupancy %","avg_adr":"ADR ($)"})
        fig2.update_layout(height=300, legend=LEGEND_STYLE, **CHART_LAYOUT)
        pchart(fig2)

    fig3 = px.pie(room_kpis, names="room_type", values="total_revenue",
        color_discrete_sequence=COLORS,
        title="Revenue Share by Room Type", hole=0.5)
...
with tab3:
    ...
    # Heatmap colorscale uses valid rgb() format
    fig_heat = px.imshow(
        pivot_table,
    ...
    fig_heat.update_layout(height=380, legend=LEGEND_STYLE,
        coloraxis_colorbar=dict(title="Occ %", tickfont=dict(color="#8a9ab5")), **CHART_LAYOUT)
    ...
    fig_heat2.update_layout(height=300, legend=LEGEND_STYLE,
        coloraxis_colorbar=dict(title="RevPAR $", tickfont=dict(color="#8a9ab5")), **CHART_LAYOUT)
...
with tab4:
    ...
    # FIXED: channel bar chart uses valid rgb() colorscale
    c1, c2 = st.columns(2)
    with c1:
        fig_ch = px.bar(channel_df, y="booking_channel", x="total_revenue",
        ...
        fig_ch.update_layout(showlegend=False, height=320,
            coloraxis_showscale=False, legend=LEGEND_STYLE, **CHART_LAYOUT)
        pchart(fig_ch)
    with c2:
        fig2 = px.scatter(channel_df, x="avg_stay", y="avg_rate",
            size="total_bookings", color="booking_channel",
            color_discrete_sequence=COLORS,
            title="Avg Stay vs Avg Rate — Bubble = Bookings",
            labels={"avg_stay":"Avg Stay (nights)","avg_rate":"Avg Rate ($)"})
    ...
    if show_forecast:
        ...
        if f"{m}_low" in fcast.columns:
            fig_f.add_trace(go.Scatter(
                ...
                y=pd.concat([fcast[f"{m}_high"], fcast[f"{m}_low"][::-1]]),
                fill="toself",
        ...
        fig_f.add_trace(go.Scatter(
            x=hist["date"], y=hist[m], name="Historical",
            line=dict(color=BLUE, width=2.5)
        ))
        fig_f.add_trace(go.Scatter(
            x=fcast["date"], y=fcast[m], name="Forecast",
        ...
        fig_f.update_layout(
            ...
            legend=dict(orientation="h", yanchor="bottom", y=1.02,
                        bgcolor="rgba(0,0,0,0)", bordercolor="rgba(201,168,76,0.13)", borderwidth=1),
            **CHART_LAYOUT
        )
        pchart(fig_f)
    ...
    with st.spinner("Analysing your KPIs with Groq AI (Llama 3.1)..."):
        try:
            ...
            <div style="background:linear-gradient(135deg,#111827,#1a2235);
                        ...
                        color:#e8dcc8;font-size:14px;line-height:1.8;">
                <span style="color:#c9a84c;font-weight:600;font-size:12px;
                             letter-spacing:1px;text-transform:uppercase;">
                AI Insight — Khoshaba Odeesho Revenue Dashboard</span><br><br>
                {insight}
            </div>
            """, unsafe_allow_html=True)
        ...
        with st.spinner("Generating executive PDF..."):
            try:
                ...
                pdf_bytes = generate_pdf_report(
                    ...
                    forecast_df=forecast_df if include_fore else None,
                    ai_insight=ai_text,
                    title=report_title,
                ...
                st.download_button(
                    ...
                    data=pdf_bytes,
                    file_name=f"hotel_revenue_report_{datetime.now().strftime('%Y%m%d')}.pdf",
...
# ── Footer ────────────────────────────────────────────────────────
st.markdown("""
<div style="text-align:center;padding:2rem 0 1rem;color:#2a3550;font-size:12px;letter-spacing:.5px">
    HOTEL REVENUE INTELLIGENCE DASHBOARD &nbsp;·&nbsp;
    DESIGNED & BUILT BY <span style="color:#c9a84c">KHOSHABA ODEESHO</span> &nbsp;·&nbsp;
    PYTHON · STREAMLIT · GROQ AI · PROPHET
</div>
""", unsafe_allow_html=True)
